...
-   "R" is considering the variable "habit" to be a character variable, or `<chr>`. "R" considers "visits" to be a number with decimals variable, or `<dbl>`
:::
...
-   There is a positive relationship between `weight` and `weeks`. This can be seen by looking at the coordinates between the x and y axis. The graph shows a positive correlation and shows that as the length of the pregnancy progresses, the birth weight of the baby increases as well.
:::
...
```{r}
ggplot(data = nc, aes(x = weeks, y = gained)) + geom_point() + labs(x = "Length of Pregnancy (in weeks)", y = "Maternal Weight Gain (lbs)", title = "Relationship between Maternal Weight Gain and Pregnancy Length")  + theme_bw()
...
**A.** Adding the argument `color = premie` added a `premie` key to the side of the graph which demonstrates that the coral colored coordinates represent full-term births and the blue colored coordinates represent `premie` births. This also added the colors to the graph's coordinates.

**B.** There are now four variables displayed on the plot: weeks of pregnancy, maternal weight gain, and whether the birth was full-term or premature.

**C.** The cutoff for `premie` babies occurs at around the 36-37 week time frame. After 37 weeks, the birth is considered full-term.
...
ggplot(data = nc, aes(x = mage, y = weight, color = gender))+ 
  geom_point() + 
  labs(x = "Mother's Age (in years)", y = "Newborn's Weight (lbs)") + 
  theme_bw()

```

-   There is very little correlation between the mother's age and the newborn's weight, however there is a slight correlation demonstrating that as the mother's age increases, so does the newborn's weight.
:::
...
**A.** `count` represents the number of total births in North Carolina in that given time frame. This can be seen by the steep increase during the 38 to 40 week period, which is when most births occur.

**B.** The average length of pregnancies in weeks appears to be from about 38 to 40 weeks.
...
**C.** After changing the binwidth to 100, there is only 1 bin that can fit in this frame and it shows about 800 cases within the single bin.
:::
...
```{r}
ggplot(data = nc, aes(x = weight)) + labs(x = "Newborn's Weight (lbs)", y = "Number of Total Births in NC", title = "Relationship between Newborn's Weight and Total Births in NC") +
...
-   The gender that appears to have the slightly larger average birth weight are male newborns.
:::
...
```{r}
ggplot(data = nc, aes(x = gained, y = mature)) + labs(x = "Maternal Weight Gained (lbs)", y = "Maternal Maturity Status", title = "Relationship between Maternal Weight Gain and Maternal Maturity") +
  geom_boxplot() + 
  theme_bw()

```

-   The median weight gain for mature mothers is higher than the median weight gain for younger mothers.
:::
...
```{r}
ggplot(data = nc, aes(x = weeks, y = habit)) + labs(x = "Pregnancy Progression (in weeks)", y = "Smoker or Non-Smoker", title = "Relationship between Pregnancy Progression and Maternal Smoking Habits") +
  geom_boxplot() + 
  theme_bw()

```

-   In terms of spread throughout the entire boxplot, including outliers, non-smoking mothers have more variable. In terms of inside the boxplots and the averages, smoking mothers have more variable.
:::
...
ggplot(data = nc, aes(x = fage)) +
  geom_histogram(binwidth = 1, color = "black", fill = "blue") + labs(x = "Father's Age") + theme_bw()

```

-   The variable for father's age is not symmetrical. The histogram skews more towards fathers between the ages of 30-35 with some high numbers at around ages 21 and 22.
:::
...
```{r}
ggplot(data = nc, aes(x = weight, y = whitemom)) + labs(x = "Newborn's Weight (in lbs)", y = "White or Non-white", title = "Relationship between Newborn's Weight and Maternal Racial Status") +
  geom_boxplot() + 
  theme_bw()

```

-   In this sample, the median birth weight for babies is greater for white moms compared to non-white moms.
:::
...
ggplot(data = nc, aes(x = weeks, y = mage)) + 
  geom_point() + 
  labs(x = "Duration of Pregnancy (in weeks)", y = "Mother's Age (in years)") +
  theme_bw()

```

-   In this sample, the relationship between the mother's age and the duration of the pregnancy doesn't have much of a negative or positive correlation. However, there is somewhat of a positive correlation, meaning that as the mother's age increases, the duration of pregnancy extends slightly. 
